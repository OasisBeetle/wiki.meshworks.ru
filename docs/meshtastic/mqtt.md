...
MQTT в Meshtastic нужен для связи между mesh‑сетью и интернет‑инфраструктурой. Через него нода может:

- отправлять mesh‑пакеты в брокер;
- получать пакеты из брокера обратно в mesh;
- работать как gateway‑нода для удалённых сетей;
- передавать данные в Home Assistant, Node‑RED, карты, боты и другие внешние системы.

Проще говоря, MQTT делает мост между LoRa‑mesh и обычным интернетом.

Сразу важно понимать главное:

- **LoRa‑шифрование** и **MQTT‑публикация** - это разные уровни;
- пакет может быть защищён в радиоэфире;
- но после обработки на gateway‑ноде в broker может попасть уже читаемое содержимое.

Поэтому MQTT не стоит считать “автоматически приватным” только потому, что Meshtastic использует шифрование по радио.

## Что видит MQTT

В Meshtastic MQTT может передавать данные в двух режимах:

- как raw/protobuf пакет;
- как уже декодированный JSON.

Разница здесь критична.

### Если используется protobuf

В broker уходит raw `MeshPacket`. Если для канала включён `encryption_enabled`, payload может остаться зашифрованным ключом канала.

Это ближе к модели “broker переносит пакет, но не обязательно понимает его содержимое”.

### Если используется JSON или интеграция уже декодировала пакет

Тогда в MQTT может попасть уже читаемое содержимое:

- текст сообщений;
- node info;
- координаты;
- телеметрия;
- другие поля, которые gateway или клиент уже успел разобрать.

Именно здесь чаще всего возникает ложное ощущение безопасности: пользователь считает, что “Meshtastic шифрует сообщения”, но забывает, что **после расшифровки на gateway данные могут публиковаться дальше в открытом виде**.

## Что это означает для личных сообщений

Личные сообщения в Meshtastic могут быть защищены лучше, чем обычный групповой трафик. Но если они проходят через MQTT, это не гарантирует, что broker их не увидит.

Более точная формулировка такая:

- DM могут быть защищены на уровне mesh‑передачи;
- но на MQTT‑слое они могут стать читаемыми, если gateway или интеграция публикует уже декодированные данные.

Поэтому пользовательский вывод “личные сообщения через MQTT не шифруются” часто отражает реальный эффект, который вы видите в системе. Просто технически правильнее говорить так:

**приватность DM может теряться не в радиоэфире, а на MQTT‑слое после расшифровки и публикации.**

## Публичный MQTT‑сервер Meshtastic

Проект Meshtastic поддерживает публичный MQTT‑сервис, к которому можно подключаться при соблюдении ряда ограничений. Он нужен для того, чтобы удалённые mesh‑сети могли связываться через интернет и не были полностью изолированы друг от друга.

Но это не “полный прозрачный мост” для всего трафика. На публичном сервере действуют ограничения, чтобы:
